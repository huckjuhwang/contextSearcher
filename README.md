...
1) 화면설계,<br>
2) 퍼블리싱, <br>
3) 검색속도 개선(하단에 설명 기재),<br>
-> 전체시스템의 모듈 작동시간 분석 및 상황에 맞는 트리거, 멀티쓰레드를 사용해 보았음 적절한 방식을 찾게되어 80퍼 이상의 성능향상의 결과를 이루었습니다. <br>
4) 추가적인 정보를 얻기 위한 통계 테이블 생성 <br>
-> 단어 사이의 빈도수를 미리 계산하여 조회성능을 향상 시켰습니다. 대량의 1천만건 이상의 데이터를 전부 insert하는 작업이 이루어 져있었기 때문에 preparedStatement에서 제공하는 addBacth() 메소드를 사용을 해보았습니다. <br>
